...
The headline shift: **the platform storefront APIs solved the problem the browser work was trying to solve.** Shopify, WooCommerce, and open-guest-API Magento hand real shipping quotes and structured product data to unauthenticated callers, so the engine-versus-wall research below now matters only for the residue — stores on none of these platforms, and vendors like Amazon whose data is reachable no other way.
...
3. **DataDome vendors (alone or stacked) need one of:** headed patchright/cloakbrowser driven invisibly via **background computer-use (cua-driver / Codex Computer Use)**, or a **non-quote data source** (published policy, carrier estimate, platform cart API). This is the main unproven step — see Open questions.
4. **Prefer avoiding the browser entirely** where possible: platform cart APIs and carrier-rate estimates are the general cold-paths that sidestep bot-walls, because they don't drive the walled HTML.
...
| **Cloudflare** (CDN vs challenge) | Often just CDN, not a hard wall; try plain then escalate | not deeply tested |
...
## Evidence — platform census & Magento validation (2026-07-30)
...
Results: **Shopify** = DERNORD (dernord.com) and Mettle Air (mettleairstore.com), both verified end-to-end with tokenless cart quotes (Mettle Air returned four live UPS-tier rates; DERNORD one flat $55 "FeDex" rate on a $10 item — vendor economics no carrier API would predict). **Magento** = SparkFun, with the guest API **open**: unauthenticated `guest-carts` → `items` (SKU `DEV-13975`) → `estimate-shipping-methods` returned live ShipperHQ rates (FedEx Ground Economy $19.31 / FedEx Ground $30.32 / UPS Ground $34.28 / 2-day tiers). **BigCommerce** = GoBilda + ServoCity (markers; rate call 403s, stays a browser case). Everything else custom; Cloudflare-walled Bolt Depot / Glacier Tanks / StepperOnline blocked the probe (unknown). WooCommerce: zero among tracked vendors.

So the answer to "workflow-changing or niche for the tracked head?" is: **niche for the head (3 of ~29 quotable), workflow-changing for the open-web tail** — which is where the path was aimed anyway. Magento is confirmed as the third unauthenticated quote platform; recipe (detection + full flow) now lives in `SKILL.md`'s pricing pass. Magento's gap vs Shopify: no unauthenticated catalog search — the SKU must come from the product page.
...
- **Salesforce Commerce Cloud and Shopware** are the next untested platforms now that Magento is validated (see the platform-census evidence section).
...
- **Shopify Web Bot Auth**: Shopify rate-limits automated traffic and gives unsigned anonymous bots the strictest limits, while inviting bot operators to sign their requests. Registering converts us from anonymous to known-good before any scaling, and is the single highest-leverage durability move on the storefront-API path.
