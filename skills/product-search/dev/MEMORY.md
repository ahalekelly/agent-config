...
The headline shift: **the platform storefront APIs solved the problem the browser work was trying to solve.** Shopify, WooCommerce, open-guest-API Magento, BigCommerce Stencil, and Squarespace hand real shipping quotes and structured product data to unauthenticated callers, so the engine-versus-wall research below now matters only for the residue — stores on other platforms, endpoint-specific walls, and vendors like Amazon whose data is reachable no other way.
...
- **General cold-path methods** (zero per-vendor setup, generalize to unknown stores — these carry the tail):
  - **Platform cart APIs — validated, and they replace the browser for most of the tail.** Shopify's Storefront GraphQL API grants **tokenless** cart read/write: `cartCreate` with the destination in `buyerIdentity`, then the exact deferred rate shape `cart(id: $id) { id ... @defer { deliveryGroups(first: 10, withCarrierRates: true) { nodes { groupType deliveryOptions { handle title code description deliveryMethodType estimatedCost { amount currencyCode } } } } } }`. Parse the complete `multipart/mixed` response, including incremental parts. The workflow was verified live across twelve stores and captures vendor economics a carrier API cannot, including free shipping, application-defined methods, LTL freight, and destination refusals. WooCommerce's Store API is likewise unauthenticated via `Cart-Token` and preserves rate and cart taxes. Shopify remains pre-tax in this flow. Magento's guest-cart REST workflow works where the store leaves it open. BigCommerce Stencil's primary workflow is anonymous `POST /api/storefront/carts`, followed by `POST /api/storefront/checkouts/<cart-id>/consignments?include=consignments.availableShippingOptions` with the cart-issued `SHOP_SESSION_TOKEN`, `X-SF-CSRF-TOKEN`, selected physical item ID, and complete destination array. It returned nonempty exact-address rates on 11/11 stores. Tokenless BigCommerce GraphQL returned 401 on 11/11; a page-emitted origin-scoped token was observed on 8/11 and searched products successfully on 8/8. The older theme-form estimator is a diagnostic only, never the primary path or an automatic fallback. Carrier APIs answer the wrong question entirely. Trap on all platforms: an empty rate list means no quote, never free.
...
## Evidence — platform census and storefront validation
