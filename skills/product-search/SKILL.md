...
5. **A browser-swarm subagent** — last. The dividing line is reading versus interacting: reach for the shared headless fingerprint-Chromium browser only when the task needs interaction, such as a cart taken to a live shipping quote, a configurator clicked through, or a form that has to be filled — or when a listing's live price and stock exist nowhere but the rendered page. It costs far more tokens and wall-clock than a search, and it is the only path that bot walls can block.
...
- **Platform storefront APIs** — use store-scoped structured product data and the merchant's own cart economics before browser work. End-to-end public quotes cover Shopify, WooCommerce, Magento, BigCommerce, and Squarespace. Product discovery also covers Wix, Ecwid, and Salesforce Commerce Cloud, whose generic destination quote remains an explicit browser boundary. `vendors.md` has the fast path and learned cache; [`platform-apis.md`](platform-apis.md) has the exact protocols and failure semantics.
...
Only when asked to compare pricing across vendors — typically a second pass after the initial search. Follow the complete pricing-pass recipe and learned shipping cache in [`vendors.md`](vendors.md); use [`platform-apis.md`](platform-apis.md) for executable workflows, protocol details, and failure semantics. The gated and non-platform residue goes to a browser-swarm subagent following `~/.agents/browser-swarm/README.md` and `~/.agents/browser-swarm/docs/bot-detection.md`.
