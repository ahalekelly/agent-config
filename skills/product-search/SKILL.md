...
4. **A server-side fetch through a GPT leaf** (Pi's `fetch_content`, Codex's cloud fetch) — the rung most easily forgotten. It runs on the provider's egress, not this machine, so it reaches retailers that block a local `WebFetch` outright, and it returns real page text rather than a search summary. Good for descriptions, specs, and published policy pages. It cannot interact, so it will not produce a cart quote, and it tends to miss the dynamically-rendered offer block — expect title, description and specs, not reliable price or stock. Try a local `WebFetch` first since it is cheaper; escalate here when it is blocked or returns a body-less shell.
