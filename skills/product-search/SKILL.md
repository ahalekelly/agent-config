...
2. **An API** — either a vendor API recorded in `vendors.md`, a marketplace pseudo-store, or the public storefront package. `cross-shop` covers Shopify, WooCommerce, Magento guest carts, BigCommerce Stencil, and Squarespace; Wix, Ecwid, Salesforce Commerce Cloud, and OpenCart have explicit browser or merchant-customization boundaries. Use `platform-apis.md` for exact contracts.
...
- **Amazon ASIN hydration** — after discovery yields an exact ASIN, pass the ASIN ref or `/dp/` URL to `cross-shop` `product` for the current title, image, rating, offer prices, sellers, shippers, and anonymous-default delivery promises. The undocumented read-only endpoint is not keyword search or an SF shipping quote. Treat an all-offers-unavailable `api_error` as endpoint unavailability, not proof that the product does not exist, and stop on blocks instead of retrying.
...
3. Batch API-capable stores through the `cross-shop` MCP tools (`search`, `product`, `quote`, `images`) when the agent has them — Claude agents do, and Pi does via pi-mcp-adapter. Agents without the MCP registration run the same commands as a CLI: `uv run --project cross-shop cross-shop search …`. Inspect candidates with `product` — a known Amazon ASIN or `/dp/` URL goes through `product` too — then quote exact refs or variant handles. Open `platform-apis.md` for protocol and failure details.
