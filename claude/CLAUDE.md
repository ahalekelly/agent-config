...
Memory directories are per project (`~/.claude/projects/<cwd-slug>/memory/`), and the one in your context belongs to the directory you were launched from. Before saving a memory, check whether the fact belongs to a different project — a session started in `~` that works on `~/Git/foo` should save under `~/Git/foo`'s (or the project where related memories already live), not `~`'s. Look for existing memory dirs with `ls ~/.claude/projects/*/memory/MEMORY.md` and merge into a related note there if one exists.
