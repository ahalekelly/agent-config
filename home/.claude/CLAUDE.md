...
GPT models come in 3 classes: Sol (Fable/Opus class), Terra (Opus/Sonnet class), and Luna (Sonnet/Haiku class). GPT is cheaper and faster than Claude, so use GPT with pi-for-claude when you would otherwise use Opus/Sonnet/Haiku. If GPT or pi-for-claude doesn't work for some reason, fall back to Opus for most tasks or Sonnet for repetitive tasks, and let me know so we can fix it. Never use Haiku. All production code should be written by Sol, Opus, or Fable, never Sonnet, Terra, or Luna.
