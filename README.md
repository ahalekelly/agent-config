...
On macOS, sync also links `.zshrc`, `.zprofile`, and the iTerm2 dynamic profile. The `com.akelly.t3-keepalive` launch agent checks every 5 minutes and reopens T3 Code if it has quit, hiding the new window; existing windows stay untouched. It requires Xcode command-line tools. Logs are in `~/Library/Logs/t3-keepalive.log`. Stop it with `launchctl bootout gui/$(id -u)/com.akelly.t3-keepalive`; running sync installs it again.
