...
For driving via MCP rather than raw scripts, the relevant third-party servers are `stealth-browser-mcp` (nodriver + CDP, standalone page driver), `patchright-mcp-lite` (drop-in patched Playwright), and AdsPower's LocalAPI MCP (a profile manager that hands back a CDP endpoint, not a page driver).

## Playwright itself is a tell

Every Playwright page — launched or attached — gets `Runtime.enable`, `Log.enable`, `Page.createIsolatedWorld`, and `Page.addScriptToEvaluateOnNewDocument` during frame-session init. Anti-bot vendors probe both signals directly; the `rebrowser-bot-detector` suite names them `runtimeEnableLeak` and `pwInitScripts`. This is a floor on how stealthy any Playwright-driven browser can be, ours included, and it can only be lifted by a patched Playwright such as rebrowser-patches.
