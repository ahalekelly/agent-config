...
Before reaching for a leaf at all, check whether a search, an API, or an MCP can answer the question: web search returns page text, so reading a page rarely needs a browser. Leaves are for interaction — forms, carts, configurators. A browser costs far more tokens and wall-clock, and it is the only path a bot wall can block.
