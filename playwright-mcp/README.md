...
Before reaching for a leaf at all, work down the cheaper sources: an MCP or working vendor API first, then web search, which on most sites returns page text and so covers reading. Leaves are for interaction — forms, carts, configurators — and for the sites that starve the cheaper paths, where search sees only a page title and `WebFetch` gets an error or a body-less shell. A browser costs far more tokens and wall-clock, and it is the only path a bot wall can block.

CDP is Chromium-only, so a different *engine* — headless Firefox, say — has to launch its own browser rather than attach. A different Chromium *build* does not: swapping the binary this script launches for a fingerprint-patched Chromium keeps the whole shared-daemon architecture and renders on Akamai-protected vendor sites, which stock Chrome for Testing does not. [Browser Engines and Vendor Bot Detection](<Browser Engines and Vendor Bot Detection.md>) covers which engine renders on which bot-detection system, the builds worth using, the launched-mode Firefox leaf config, and how to identify the system a site is running. Note that every context on one daemon shares a single fingerprint, so all attached leaves look like one device.
