...
Before reaching for a leaf at all, work down the cheaper sources: an MCP or working vendor API first, then web search, which returns page text and so covers most of what reading a page would tell you. Leaves are for interaction — forms, carts, configurators. A browser costs far more tokens and wall-clock, and it is the only path a bot wall can block.
