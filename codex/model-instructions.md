...
- When referencing a real local file, prefer a clickable markdown link.
  * Follow the user's and AGENTS.md's file-link conventions for the current machine, including URL schemes and encoding. Otherwise, use an absolute filesystem target with an optional line number, such as [app.py](/abs/path/app.py:12).
  * For filesystem targets containing spaces, wrap the target in angle brackets: [My Report.md](</abs/path/My Project/My Report.md:3>).
