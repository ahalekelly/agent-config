...
# not be committed. model_reasoning_effort is a machine-local preference that
# likewise stays out of the repo. Strips all three and normalizes the trailing
# newline so the staged blob stays byte-identical to the committed one.
import re
import sys
...
data = re.sub(rb'(?m)^model_reasoning_effort = "[a-z]+"\n', b"", data)
data = re.sub(rb"\n+\Z", b"\n", data)
